...
//public:
//	dielectic(float ior) : ir(ior) {} //ior - index of refaction
//
//	virtual bool scatter(
//		const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
//		attenuation = color(1, 1, 1);
//		float refraction_ratio = rec.front_face ? (1.0 / ir) : ir;//if not hitting from the front then IOR ratio is reciprocal
//
//		vec3 unit_direction = unit_vector(r_in.direction());
//		//vec3 refracted = refract(unit_direction, rec.normal, refraction_ratio);
//
//		float cos_theta = fmin(dot(-unit_direction, rec.normal), 1.0);//from dot product, cannot be greater than 1
//		float sin_theta = sqrt(1 - cos_theta * cos_theta);//from trig equality sin = sqrt(1 - cos^2(theta))
//
...
//
//private:
//	static float reflectance(float cosine, float ref_idx) {//for Fresnel
